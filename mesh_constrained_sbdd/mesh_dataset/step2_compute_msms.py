...
def find_xyzrn_files(directory):
    """
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith('.xyzrn'):
                xyzrn_files.append(os.path.join(root, file))
    return xyzrn_files
...
def read_msms(data_dir, prefix):
    ...
    # vertices
    with open(mesh_prefix + '.vert', 'rb') as f:
        raw_data = f.read()
        encoding = chardet.detect(raw_data)['encoding']


    with open(mesh_prefix + '.vert',encoding=encoding) as f:
    ...
    # faces
    with open(mesh_prefix + '.face', encoding=encoding) as f:
...
charge_set = set([1, 6, 7, 8, 9, 15, 16, 17])
...
def compute_msms(data_root, gdb_id, msms_bin):
    ...
    xyzrn_files = find_xyzrn_files(data_dir)
    assert len(xyzrn_files) == 1
    # # xyzrn file
    # xyzrn_file = os.path.join(data_dir, f'{gdb_id}.xyzrn')
    xyzrn_file = xyzrn_files[0]
    # info = read_xyzrn_file(xyzrn_file)
    # xyz = info[0][:3]
    # """
    # add
    # """
    # global npz_file_count
    ...
    # distances = np.sqrt(np.sum((verts - xyz)**2, axis=1))
    ...
    # save MSMS mesh
    data_dir_save = os.path.join("/mnt/workspace/dlly/targetdiff_mesh/data/crossdocked_pocket10_mesh4",gdb_id)
    os.makedirs(data_dir_save, exist_ok=True)

    #
    xyz_file_name_with_extension = os.path.basename(xyzrn_file)
    xyz_file_name = xyz_file_name_with_extension.split('.')[0]
    mesh_file = os.path.join(data_dir_save, f'{xyz_file_name}_msms.npz')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-root', type=str, default='/mnt/data/oss_beijing/mesh/targetdiff_mesh/data/crossdocked_pocket10_mesh4')
    parser.add_argument('--msms-bin', type=str, default='/mnt/workspace/mesh/MSMS/msms.x86_64Linux2.2.6.1')
    ...
    # """add"""
    # print(f'Total number of .npz files saved: {npz_file_count}')
