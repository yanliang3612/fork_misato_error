...
def convert_to_xyzrn(split,data_file_sdf_list,src_fpath, out_root):
    for i in range(len(data_file_sdf_list)):
        ...
        xyzrn_path = os.path.join(out_root, f'{data_file}.xyzrn')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out-root', type=str, default='/mnt/data/oss_beijing/mesh/targetdiff_mesh/data/crossdocked_pocket10_mesh_icml_new')
    ...
    # for split in ['train']:
    split = 'train'
    src_fpath = os.path.join("/mnt/workspace/dlly/targetdiff_mesh/data/", 'data_file_sdf_list.pt')
