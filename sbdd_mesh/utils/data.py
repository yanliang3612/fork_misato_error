...
def find_mesh_points_close_to_vdw_radius(atom_coordinates, mesh_points, elements, vdw_radii_dict,num,th):
    close_points_indices = []
    close_points_coordinates = []

    for i, atom in enumerate(atom_coordinates):
        atomic_number = elements[i]

        vdw_radius = vdw_radii_dict[atomic_number]

        distances = np.linalg.norm(mesh_points - atom, axis=1)

        indices = np.where((distances >= vdw_radius - th) & (distances <= vdw_radius + th))[0]
        assert len(indices) >= num
        indices = indices[:num]
        close_points_indices.append(indices)
        close_points_coordinates.append(mesh_points[indices])

    return close_points_indices, close_points_coordinates


def find_mesh_points_close_to_vdw_radius_abs(atom_coordinates, mesh_points, elements, vdw_radii_dict,num, th):
    close_points_indices = []
    close_points_coordinates = []
    ...
    for i, atom in enumerate(atom_coordinates):
        ...
        sorted_indices = np.argsort(np.abs(distances - vdw_radius))

        closest_indices = sorted_indices[:num]
        close_points_indices.append(closest_indices)
        close_points_coordinates.append(mesh_points[closest_indices])
    return close_points_indices, close_points_coordinates




def parse_sdf_npz_file(path, npz_file, num, th):
    fdefName = os.path.join(RDConfig.RDDataDir, 'BaseFeatures.fdef')
    factory = ChemicalFeatures.BuildFeatureFactory(fdefName)
    # read mol
    if path.endswith('.sdf'):
    ...
    mesh_verts = (np.load(npz_file)['verts']).astype(np.float32)
    indices, mesh_coordinates = find_mesh_points_close_to_vdw_radius_abs(pos, mesh_verts, element, vdw_radii_dict,num,th)
    mesh_coordinates = np.vstack(mesh_coordinates).astype(np.float32)
    mesh_feat_mat = np.repeat(feat_mat, num, axis=0).astype(np.float32)
    ...
    data = {
        ...
        'atom_feature': feat_mat,
        'hybridization': hybridization,
        'mesh_pos': mesh_coordinates,
        'mesh_feat_mat': mesh_feat_mat,
    }
    return data
