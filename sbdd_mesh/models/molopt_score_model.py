...
def center_pos_with_mesh(protein_pos, ligand_pos, ligand_mesh_pos, batch_protein, batch_ligand,batch_ligand_mesh,mode='protein'):
    if mode == 'none':
        offset = 0.
        pass
    elif mode == 'protein':
        ...
        protein_pos = protein_pos - offset[batch_protein]
        ligand_pos = ligand_pos - offset[batch_ligand]
        ligand_mesh_pos = ligand_mesh_pos - offset[batch_ligand_mesh]
    else:
        raise NotImplementedError
    return protein_pos, ligand_pos, ligand_mesh_pos, offset
...
def calculate_vdw_loss(pred_mesh, pred, batch_ligand_element, vdw_radii_dict, batch_ligand,num):
    loss_list = []


    for i, atom in enumerate(pred):
        vdw_radius = vdw_radii_dict[batch_ligand_element[i]]

        mesh_points = pred_mesh[i*num:(i+1)*num]


        distances = torch.norm(mesh_points - atom, dim=1)


        loss = torch.square(torch.abs(distances - vdw_radius))
        loss_list.append(loss.mean())

    total_loss = torch.stack(loss_list)

    loss = scatter_mean(total_loss, batch_ligand, dim=0)
...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, args, protein_atom_feature_dim, ligand_atom_feature_dim):
        super().__init__()
        config = args
        self.config = args
        ...
        self.loss_mesh_weight = config.loss_mesh_weight
    ...
    def get_diffusion_loss(
            self, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, ligand_mesh_pos, ligand_mesh_v, batch_ligand, batch_ligand_mesh, batch_ligand_element, time_step=None
    ):
        num_graphs = batch_protein.max().item() + 1
        # protein_pos, ligand_pos, _ = center_pos(
        #     protein_pos, ligand_pos, batch_protein, batch_ligand, mode=self.center_pos_mode)
        protein_pos, ligand_pos, ligand_mesh_pos, offset = center_pos_with_mesh(protein_pos, ligand_pos, ligand_mesh_pos, batch_protein, batch_ligand, batch_ligand_mesh,
        ...
        a_pos_mesh = a[batch_ligand_mesh].unsqueeze(-1)
        # pose_noise_mesh = pos_noise.repeat_interleave((int(ligand_mesh_pos.shape[0]/ligand_pos.shape[0])),dim=0)

        pos_noise_mesh = torch.zeros_like(ligand_mesh_pos)
        pos_noise_mesh.normal_(ligand_mesh_pos)

        ligand_pos_perturbed_mesh = a_pos_mesh.sqrt() * ligand_mesh_pos + (1.0 - a_pos_mesh).sqrt() * pos_noise_mesh

        log_ligand_mesh_v0 = index_to_log_onehot(ligand_mesh_v, self.num_classes)
        ligand_v_perturbed_mesh, log_ligand_vt_mesh = self.q_v_sample(log_ligand_mesh_v0, time_step, batch_ligand_mesh)
        ...
        preds_mesh =self(
            ...
            init_ligand_pos=ligand_pos_perturbed_mesh,
            init_ligand_v=ligand_v_perturbed_mesh,
            batch_ligand=batch_ligand_mesh,
            time_step=time_step
        )
        ...
        pred_ligand_pos_mesh, pred_ligand_v_mesh = preds_mesh['pred_ligand_pos'], preds_mesh['pred_ligand_v']
        pred_pos_noise_mesh = pred_ligand_pos_mesh - ligand_pos_perturbed_mesh
        ...
        if self.model_mean_type == 'C0':
            target_mesh, pred_mesh = ligand_mesh_pos, pred_ligand_pos_mesh
        elif self.model_mean_type == 'noise':
            target_mesh, pred_mesh = pose_noise_mesh, pred_pos_noise_mesh
        else:
            raise ValueError
        ...
        loss_mesh = calculate_vdw_loss(pred_mesh, pred, batch_ligand_element, vdw_radii_dict, batch_ligand, int(ligand_mesh_pos.shape[0]/ligand_pos.shape[0]))
        loss_mesh = torch.mean(loss_mesh)
        ...
        loss = loss_pos + loss_v * self.loss_v_weight + loss_mesh * self.loss_mesh_weight
