...
def evaluate(sample_path):
            ...
            try:
                ...
                if docking_mode == 'qvina':
                    ...
                    qvina_task = QVinaDockingTask.from_generated_mol(
                        mol, r['data'].protein_filename,  protein_root=protein_root)
    ...
    evaluate_results = {
        ...
        'Vina_Score_mean': np.mean(vina_score_only),
        'Vina_Score_median': np.median(vina_score_only),
        'Vina_Min_mean': np.mean(vina_min),
        'Vina_Min_median': np.median(vina_min),
        'Diversity_mean':   np.mean(diversity_list),
        'Diversity_median': np.median(diversity_list),
        'high_affinity_mean':  np.mean(high_affinity),
        'high_affinity_mean':  np.median(high_affinity)
    }
